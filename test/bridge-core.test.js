...
test("exposes exactly the three original tools with strict object schemas", async (t) => {
  const { core } = await fixture(t);
  assert.deepEqual(core.tools.map(({ name }) => name), ["list_files", "read_file", "write_file"]);
